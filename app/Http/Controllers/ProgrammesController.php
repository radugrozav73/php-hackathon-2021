...
class ProgrammesController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $roomInstances = Programmes::select('room_name','start_date', 'end_date')->get();
        if($date2->gt($date1)){
            return response('Start date greater than end date', 401);
        }

        foreach($roomInstances as $instances){
            if(($date1->gte($instances->start_date) && $date1->lte($instances->end_date)) || ($date2->gte($instances->start_date) && $date2->lte($instances->end_date)) && $instances->room_name === $request->room_name){
                $nrOfOccurences++;
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach($users as $user){
            foreach($user->programmes as $programs){
                if (($date1->gte($programs->start_date) && $date1->lte($programs->end_date)) || ($date2->gte($programs->start_date) && $date2->lte($programs->end_date))){
